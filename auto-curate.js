...
const SPICE_WORDS = [
  ['🌶️🌶️🌶️ Hot', ['낙곱새','불닭','매운갈비찜','불족발','아구찜','해물찜','쭈꾸미',
                   '주꾸미','매운탕','닭발','매운족발','불막창','매운낙지','짬뽕',
                   '불곱창','화끈','매운','불맛','땡초','청양','뽈찜','찜닭','갈비찜']],

  ['🌶️🌶️ Medium', ['떡볶이','비빔냉면','비빔밀면','제육','오징어볶음','낙지볶음',
                   '순두부','김치찌개','부대찌개','닭갈비','양념치킨','고추장',
                   '비빔국수','열무','알탕','동태찌개','짜글이','곱창전골',
                   '해물찜','생선조림','갈치조림','고등어조림','비빔밥']],

  ['🌶️ Mild',     ['국밥','설렁탕','곰탕','칼국수','수육','백반','만두','돈까스',
                   '초밥','스시','물냉면','물밀면','물회','삼계탕','갈비탕','추어탕',
                   '우동','소바','덮밥','카레','김밥','전골','샤브','보쌈','족발',
                   '갈비','삼겹살','한우','회','횟집','조개구이','장어','어묵',
                   '죽','한정식','정식','밥상','쌈밥','콩나물국밥','재첩국','복국']],
];
...
const SOLO_GROUPS = [
  ...
  { name:'고기구이',    likely:false,
    words:['삼겹살','목살','갈비','숯불','구이','한우','생고기','막창','곱창','대창'] },
...
async function main(){
    ...
    if (!p.spice_level && p.category === 'restaurant'){
      for (const [level, words] of SPICE_WORDS){
        if (has(text, words)){
          ...
          sql.push(`update places set spice_level = '${level}' where id = '${p.id}';   -- ${p.name_ko}`);
          break;
        }
      }
    }
  ...
  const bySpice = {};
  spiceHits.forEach(h => { bySpice[h.level] = (bySpice[h.level] || 0) + 1; });
